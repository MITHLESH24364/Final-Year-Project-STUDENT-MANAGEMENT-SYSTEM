...
//     setDisplayData(filteredStudents);
//   };
...
//     try {
//       let url = "/api/absent-all";
//       if (dateFilter) {
//         url = `/api/absent-by-date?date=${dateFilter}`;
//       }
...
//         if (orderBy === 'sno') return a.id - b.id;
...
//   useEffect(() => {
//     fetchAbsentStudents();
//   }, [dateFilter, classFilter, sectionFilter, orderBy]);
...
//         <button onClick={fetchAbsentStudents}>Filter</button>
//       </div>
...
// import React, { useState, useEffect } from 'react';
// // import studentData from "../Data/studentData"; // Remove this import if using API
...
//   // Fetch absent students data from backend
...
//     const sectionParam = sectionFilter ? `&section=${sectionFilter}` : '';
    
//     const response = await fetch(`/sms/attandence/absent-by-date?${dateParam}`, {
//       headers: {
...
const AbsentStudents = () => {
  ...
  const fetchAbsentData = async () => {
    ...
    const response = await fetch(`/sms/attandence/absent-by-date?${params.toString()}`, {
      headers: {
        Authorization: `Basic ${authToken}`,
        "Content-Type": "application/json",
      },
